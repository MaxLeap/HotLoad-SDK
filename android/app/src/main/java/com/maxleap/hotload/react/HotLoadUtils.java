...
public class HotLoadUtils {
    ...
    public static WritableArray convertJsonArrayToWritable(JSONArray jsonArr) {
        ...
        for (int i=0; i<jsonArr.length(); i++) {
            ...
            try {
                ...
                // Should not happen.
                throw new HotLoadUnknownException(i + " should be within bounds of array " + jsonArr.toString(), jsonException);
            }
            ...
            if (obj instanceof JSONObject)
                ...
                throw new HotLoadUnknownException("Unrecognized object: " + obj);
        }
    ...
    public static WritableMap convertJsonObjectToWritable(JSONObject jsonObj) {
        ...
        while(it.hasNext()){
            ...
            try {
                ...
                // Should not happen.
                throw new HotLoadUnknownException("Key " + key + " should exist in " + jsonObj.toString() + ".", jsonException);
            }
                ...
                case Number:
                    ...
                    if ((number == Math.floor(number)) && !Double.isInfinite(number)) {
                        ...
                        try {
                            ...
                            throw new HotLoadUnknownException("Unable to put value " + arr.getDouble(i) + " in JSONArray");
                        }
                    }
                    break;
                case Boolean:
            ...
            try {
                switch (type) {
                    ...
                    case Null:
                        ...
                        break;
                    default:
                        throw new HotLoadUnknownException("Unrecognized type: " + type + " of key: " + key);
                }
            } catch (JSONException jsonException) {
                throw new HotLoadUnknownException("Error setting key: " + key + " in JSONObject", jsonException);
            }
        }
    ...
    public static WritableMap getWritableMapFromFile(String filePath) throws IOException {
        ...
        try {
            ...
            throw new HotLoadMalformedDataException(filePath, jsonException);
        }
    }

    public static void log(String message) {
        Log.d(REACT_NATIVE_LOG_TAG, "[HotLoad] " + message);
    }
    ...
    public static void writeReadableMapToFile(ReadableMap map, String filePath) throws IOException {
        JSONObject json = HotLoadUtils.convertReadableToJsonObject(map);
